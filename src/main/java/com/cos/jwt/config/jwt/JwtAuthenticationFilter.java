...
//            }
    ...
    // 인증이 정상적으로 완료되면 access token과 refresh token을 생성해서 응답에 담는다.
    @Override
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        // access token은 header에 넣어서 전달한다.
        response.addHeader(JwtTokenProvider.ACCESS_HEADER, JwtTokenProvider.TOKEN_PREFIX + accessToken);

        // refresh token은 자바스크립트에서 직접 접근하지 못하도록 HttpOnly Cookie로 전달한다.
        response.addHeader(HttpHeaders.SET_COOKIE, jwtTokenProvider.createRefreshTokenCookie(refreshToken).toString());
    }
}
